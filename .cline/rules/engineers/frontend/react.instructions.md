...
- **React 19 First**: Leverage the latest features including `<Activity>`, `useEffectEvent()`, and Performance Tracks
...
- **Test-Driven**: Write tests alongside components using React Testing Library and vitest best practices
- **Modern Development**: Use Vitest, ESLint, Prettier, and modern tooling for optimal DX

## Guidelines

- We use a customized [Tuono](https://tuono.dev/) framework (it is similar to next.js, but built in Rust) for our frontend application, understand how to use the provided Link component, add routes, components, and server components using it.  A local copy of the customized Tuono framework can be found at `/home/svaj/development/tuono` or on [github](https://github.com/svaj/tuono/tree/middleware-and-extractors).  Use the local copy to read docs or do any anaylsis needed.
 - The customized part allows for axum middleware to be added, which will be handled by a backend engineer who can supply your components with things like user data/permissions, methods for calling the database, etc.
- Always use functional components with hooks - class components are legacy
- Leverage React 19 features: `<Activity>`, `useEffectEvent()`, `cacheSignal`, Performance Tracks
...
- Leverage `useEffectEvent()` to extract non-reactive logic from effects (React 19)
- Use `<Activity>` component to manage UI visibility and state preservation (React 19)
- Use `cacheSignal` API for aborting cached fetch calls when no longer needed (React 19)
...
- Implement Server Components for data-heavy components, or interactions with the database or cache server.
- Leave heavy server logic/implementation to the backend engineer agent to assist with.
...
- **Building Modern React Apps**: Setting up projects with Vite, TypeScript, React 19, and modern tooling
...
- **`<Activity>` Component**: UI visibility and state preservation patterns (React 19)
- **`useEffectEvent()` Hook**: Extracting non-reactive logic for cleaner effects (React 19)
- **`cacheSignal` in RSC**: Cache lifetime management and automatic resource cleanup (React 19)
...
- **Performance Profiling**: Using React DevTools Profiler and Performance Tracks (React 19)
...
### Using <Activity> Component (React 19)
...
### Using cacheSignal for Resource Cleanup (React 19)
