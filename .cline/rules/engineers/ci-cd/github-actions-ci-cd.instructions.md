...
- **Guidance for Copilot:**
    - When adding or modifying a workflow (or any file in the repository), create a new git branch from the `develop` branch.  Make your changes to the workflow there, commit your changes, push your changes, and then open a pull request so your changes may be reviewed before they get merged in.
...
- **Guidance for Copilot:**
    ...
    - Ensure `uses` makes use of the most up to date version of an action, that will still be compatible with the application.
...
- **Deeper Dive:**
    - **`uses`:** Referencing marketplace actions (e.g., `actions/checkout@v4`, `actions/setup-node@v3`) or custom actions. Always to a major version tag (e.g., `@v4`). Avoid pinning to `main` or `latest`.
...
### **3. Dependency Review and Software Composition Analysis (SCA)**
...
### **5. Secret Scanning and Credential Leak Prevention**
...
### **6. Immutable Infrastructure & Image Signing**
...
- **Guidance for Copilot:**
    - Utilize `strategy.matrix` only to test applications against different environments, programming language versions, or operating systems concurrently.  Our application should normally be built with just one environment, one version of a language, etc.  Do not use Matrix for normal pipelines/workflows.
...
- **Guidance for Copilot:**
    ...
    - Make use of caching actions and/or artifacts to cache things like compiled rust dependencies or node_modules from javascript projects, only updating the cache if a dependency changed in a crate's `Cargo.lock` file (rust crate) or `package-lock.json` file (javascript project).
...
- **Guidance for Copilot:**
    - IMPORTANT! For now, do not implement any integration test pipelines while we have not deployed to any environment.  Only if there are environments defined is when we can utilize integration tests.
...
- **Guidance for Copilot:**
    - IMPORTANT! For now, do not implement any end-to-end test pipelines while we have not deployed to any environment.  Only if there are environments defined is when we can utilize end-to-end tests.
...
- **Guidance for Copilot:**
    - IMPORTANT! For now, do not implement any load test pipelines while we have not deployed to any environment.  Only if there are environments defined is when we can utilize load tests.
...
### Precursor
You may ignore the deployment strategies for now, as we do not have a deployment environment defined/configured.
...
- [ ] **Optimization and Performance:**
    - Is caching (`actions/cache`) and/or artifacts effectively used for package manager dependencies (`node_modules`, `~/.cargo/`, `target/`, caches) and build outputs?
    - Are cache `key` and `restore-keys` designed for optimal cache hit rates (e.g., using `hashFiles`)?
    - Is `strategy.matrix` used for parallelizing tests or builds across different environments, language versions, or OSs when testing a new environment/language setting?
...
### **7. General Github Action workflow debugging tips**

- Determine the workflow and action(s) failing.
    - if an application build, lint, or test step is failing, first diagnose if this is limited to the github actions environment.  Checkout the branch locally, run the commands the pipeline would and see if they run successfully.  If they do not, open a github issue documenting the failure in the build pipeline for the engineer agents to investigate.  
    - If those commands/steps ran successfully locally, but failed in a pipeline, try to recreate the failure using a local github action runner, available via the MCP act-testing-mcp.
    - Attempt to diagnose the exact issue with the pipeline and create a new branch to implement the fix in.  Change the pipeline as necessary, commit the changes, open a pull request against the `develop` branch so your changes may be reviewed before they get merged in.
    - If you fail at determining the cause of a failing action, Open a github issue and document the issue, what steps you took in your investigation and any findings you may have.
